...
class Device
  constructor: (attributes={}, options={}, dependencies={}) ->
    @devices = dependencies.database?.devices ? require('../database').devices
    @getGeo = dependencies.getGeo ? require './get-geo'
    @generateToken = dependencies.generateToken ? require '../generate-token'
    @clearCache = dependencies.clearCache ? require '../clear-cache' unless options.useRedis
    @clearCache = dependencies.clearCacheRedis ? require '../clear-cache-redis' if options.useRedis
